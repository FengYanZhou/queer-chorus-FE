$(document).ready(function() {
  ...
  var boxWidth = $(".bigbox").width() / 3;  //视窗宽度除以3获得移动宽度
  ...
  var leftCriticalPoint2 = "-" + virtual;
  ...
  function moving() {
    flag = false;
    if(Item[0].style.left >= leftCriticalPoint2) {
  ...
  $(".control-box li").click(function() {
    ...
    if(!Item.is(":animated") && flag) {
      ...
      if($(this).index() == 1) {
        ...
        Item.animate({ left: '+=' + move }, speed, function() {
          if(parseInt(Item[0].style.left) == 0) {
            Item[0].style.left = leftCriticalPoint;
          }
          callback = setInterval(moving, time);
        });
